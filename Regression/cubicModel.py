...
def main() :
    ...
    #Calculating the values for the cubicmodel
    m = len(x_Train)
    ...
    #Making the matrices
    matrix_A = np.array([[m,sum_x,sum_x_square,sum_x_cube],
                        [sum_x,sum_x_square, sum_x_cube,sum_x_four],
                        [sum_x_square, sum_x_cube, sum_x_four,sum_x_five],
                        [sum_x_cube, sum_x_four,sum_x_five,sum_x_six]
                        ])
    ...
    y_TestPred = inMatA_dot_matB[0] + np.dot(x_TestPred, inMatA_dot_matB[1]) + np.dot(np.square(x_TestPred), inMatA_dot_matB[2]) + np.dot(np.power(x_TestPred,3), inMatA_dot_matB[3])
    ...
    y_TrainPred = inMatA_dot_matB[0] + np.dot(x_TrainPred, inMatA_dot_matB[1])+np.dot(np.square(x_TrainPred), inMatA_dot_matB[2])+np.dot(np.power(x_TrainPred,3), inMatA_dot_matB[3])
