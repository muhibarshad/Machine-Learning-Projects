...
def main() :
    ...
    #Calculating the values for the fiveDegree model
    m = len(x_Train)
    ...
    #Making the matrices
    matrix_A = np.array([[m,sum_x,sum_x_square,sum_x_cube,sum_x_four,sum_x_five],
                        [sum_x,sum_x_square, sum_x_cube,sum_x_four,sum_x_five,sum_x_six],
                        [sum_x_square, sum_x_cube, sum_x_four,sum_x_five,sum_x_six,sum_x_seven],
                        [sum_x_cube, sum_x_four,sum_x_five,sum_x_six,sum_x_seven,sum_x_eight],
                        [sum_x_four, sum_x_five,sum_x_six,sum_x_seven,sum_x_eight,sum_x_nine],
                        [sum_x_five, sum_x_six,sum_x_seven,sum_x_eight,sum_x_nine,sum_x_ten]
                        ])
    ...
    y_TestPred = inMatA_dot_matB[0] + np.dot(x_TestPred, inMatA_dot_matB[1]) + np.dot(np.square(x_TestPred), inMatA_dot_matB[2]) + np.dot(np.power(x_TestPred,3), inMatA_dot_matB[3])+ np.dot(np.power(x_TestPred,4), inMatA_dot_matB[4])+ np.dot(np.power(x_TestPred,5), inMatA_dot_matB[5])
    ...
    y_TrainPred = inMatA_dot_matB[0] + np.dot(x_TrainPred, inMatA_dot_matB[1])+np.dot(np.square(x_TrainPred), inMatA_dot_matB[2])+np.dot(np.power(x_TrainPred,3), inMatA_dot_matB[3])+np.dot(np.power(x_TrainPred,4), inMatA_dot_matB[4])+np.dot(np.power(x_TrainPred,5), inMatA_dot_matB[5])
